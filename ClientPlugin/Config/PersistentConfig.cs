...
namespace ClientPlugin
{
    ...
    public class PersistentConfig<T> : IDisposable where T : class, INotifyPropertyChanged, new()
    {
        ...
        public static PersistentConfig<T> Load(string path)
        {
            ...
            {
                MyLog.Default.Error($"Failed to load configuration file: {{0}}; {e}", path);
                try
                {
                    var timestamp = DateTime.Now.ToString("yyyyMMdd-hhmmss");
                    var corruptedPath = $"{path}.corrupted.{timestamp}.txt";
                    MyLog.Default.Info("Moving corrupted configuration file: {0} => {1}", path, corruptedPath);
            ...
            MyLog.Default.Info("Writing default configuration file: {0}", path);
